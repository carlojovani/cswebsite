"""tools/debug_kill_phase_site_mismatch.py

Usage (PowerShell):
  python manage.py shell -c "from tools.debug_kill_phase_site_mismatch import main; main()"
...
from faceit_analytics.services.demo_events import (
    _assign_kill_phase,
    _compute_site_centers_from_plants,
    _distance_to_point,
...
def main() -> None:
    ...
    plant_centers = _compute_site_centers_from_plants([parsed], parsed.map_name or map_name)
    centers = plant_centers or _site_centers(parsed.map_name or map_name, config)

    print(f"Plant-derived centers: {plant_centers}")
    print(f"Effective centers: {centers}")

    print("CT kill samples (first 30):")
    print("round\ttime\tx\ty\tdist_a\tdist_b\tassigned_phase")
    ...
    for round_number in sorted(rounds):
        ...
        for kill in round_kills:
            ...
            print(
                f"{round_number}\t{kill.get('time')}\t{kill_x}\t{kill_y}"
